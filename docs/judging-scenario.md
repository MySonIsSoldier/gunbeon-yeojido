...
현재 공식 접속 주소는 **https://gunbeon.gangwon.kr/login**이다. 9/15 새 도메인의 HTTPS·Google/Naver 로그인·기존 예시 여행 복원·지도를 확인했다. [전환 검증](../reports/custom_domain_live_2026-09-15.md). 지정 일반 심사 계정의 최종 새 세션 리허설은 제출 직전에 별도로 수행한다.
...
5. ‘여유 조정’에서 한 곳 생략 또는 체류시간 감소 전후의 복귀 여유·도보 추정을 비교한다. 적용 후 저장하고 필요하면 되돌린다. 교통 실측·예약 확정·군 복귀 보장으로 표현하지 않는다.
