> **9/21 최신 안내:** 민준/openapi의 첫 화면과 상단 ‘여행 가이드 다시 보기’는 **기능별로 따라 해볼게요**를 먼저 엽니다. 일정 열어보기 → 필요하면 수정 → 동행 그룹/한 수 받기를 안내하세요. 마지막 ‘여행 당일 화면 살펴보기’는 출타를 시작하지 않습니다. 실제 시작은 현재 출타에서 여행을 고른 뒤 확인합니다. 20초 애니메이션은 안내 아래에서 선택합니다. [검증 기록](../reports/planning_first_guide_2026-09-21.md).

# 9/21 공개 테스트 계정 동작 변경

로그인 화면에서 민준 또는 아래 작은 **openapi 테스트 계정 넣기**를 선택한다. 두 계정 모두 로그인마다 독립된 예시 여행·그룹으로 시작한다. 같은 기기에서 로그인 상태를 유지하면 편집 내용은 새로고침 후에도 보이고, 로그아웃/재로그인 또는 다른 기기의 로그인에서는 초기 예시가 표시된다. 개인 계정의 장기 저장과 구분한다.

openapi의 3개 준비 여행(빈 일정 포함)·1개 기록·가족/친구 2개 그룹으로 일정 보기→수정→저장→새로고침→그룹 편집을 검사할 수 있다. 로그아웃 후 다시 로그인해 예시 복구를 확인한다. 실제 지인 초대와 소셜 계정 연결은 개인 계정에서 이용한다. 아래 날짜별 자료는 당시 시나리오이며 지속 저장하는 공유 심사 계정 관련 설명은 이 최신 동작으로 대체한다.
...
사용자는 1차 심사 완료 후 후속 개선을 요청했다. 아래 과거 제출 마감은 당시 기록이다. 지정 일반 심사 계정은 재생성·초기화하지 않는다.

- 지정 계정 로그인 후 20초 안내가 자동 재생된다. 일시정지·장면 선택·건너뛰기와 모션 감소를 지원한다.
- ‘준비된 일정 열기’는 계정에 저장된 실제 여행을 읽기 화면으로 연다. ‘기능별로 따라 해볼게요’는 5단계 실제 화면에 연결한다. 변경은 사용자가 저장할 때 반영된다.
- 접경 5군 각 6개, 춘천·속초 각 3개 코스를 제공한다. 관문 코스는 실제 관광정보 연결 시 표시된다.
- 장소 찾기 → 종류/이름 검색 → 상세 확인 → 이 장소로 일정 만들기 → 날짜·시간·추가 장소 → 저장. 둘러보기에는 날짜 입력을 넣지 않는다.
- 접속이 늦으면 6초 뒤 재연결 안내가 나오고 개인 상태 조회는 20초에서 실패 안내로 전환된다. 데이터가 없다고 간주해 새 상태를 저장하지 않는다.

[새 화면과 검증](../reports/content_guided_entry_2026-09-21.md)
