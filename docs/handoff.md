...
- 첫 테스트 로그인과 상단 가이드 재열기는 ‘기능별로 따라 해볼게요’가 기본이다. 20초 애니메이션은 선택형이다. 첫 동작은 일정 보기, 일정 읽기 화면에서는 일정 수정이 주 행동이며 출타 시작은 보조 동작이다.
- 가이드의 마지막 단계는 현재 출타의 설명 화면을 연다. 확인창이나 실제 출타를 자동 실행하지 않는다. 진행 중 출타 이어보기·그룹 가져오기·사용자가 직접 시작하는 절차는 보존했다.
- 합성된 취소 버튼에도 공통 스타일을 유지해 확인창 버튼 높이를 44px로 맞췄다. 긴 확인창은 화면 안에서 스크롤하고 완료 창은 제목에 초기 포커스를 둔다.
- TypeScript·121개 단위·개발/운영 빌드 통과. Chrome/Chromium 320/430/1440px 두 테스트 계정 12개 흐름, 비회원/신규 사용자, 일정 읽기/편집, 여유 조정/완료/공유 시나리오, 공개 가이드 통과. 공식 운영 주소 Chromium 3크기×2계정 6개 흐름 통과. 물리 휴대폰/Edge 검사는 아니다. 지도/관광 API 실제 성공이나 접속 지연 해결을 새로 주장하지 않는다.
- GitHub 구현 `db81f91`, 검사 보완 `50d667d`. [PR #34](https://github.com/MySonIsSoldier/gunbeon-yeojido/pull/34)는 전체 CI 통과 후 병합 완료(`a5394f8f3091d997a410ec10cddc933cef44e972`). [브라우저 전체 검사](https://github.com/MySonIsSoldier/gunbeon-yeojido/actions/runs/35570599684) 8분 22초 통과. 현재 master에서 인계/검증 자료를 후속 commit/push한다. 최종 문서 SHA는 `git log -1`과 원격 master 일치를 확인한다.
- Sites source 개발 `f84cb63ea5deda6e9c244b4b98728a4120d079a6`, 운영 `9e2e54e7e6c2361fdd1ca155ef3b19795f4b11ed`. [배포 응답](../reports/qa/planning-first-guide/deployment.json). 이후 GitHub의 검사 대기 보완은 테스트 스크립트만 바꾸며 배포 앱 코드는 같다. D1·Secret·공개 범위 변경 없음.

**다음 첫 행동:** 실제 휴대폰에서 새 사용자가 일정 보기→계획 수정까지 이해하는지 관찰한다. 상단 안내의 필요 단계 선택과 처음부터 출타를 시작하지 않는 구분을 확인한다. 과거 자동 애니메이션 우선 안내는 이 변경으로 대체한다. 최초 접속 지연과 운영 DB 백업 후속 과제는 별도 유지한다.

# 9/21 최신: 형식 유지 기능설명서 v3.3

사용자가 기존 기술/기능설명서를 현재 구현과 대조하고 형식을 바꾸지 않은 수정본을 요청했다. **v3.3·18쪽** PDF/PPTX를 별도 제작했다. [검토와 전달 파일](submission/2026-round1/editorial-review-v3-3.md).

- 5/6/7/9/16쪽: 추천 코스 36개, 국문 7개 장소 유형·추가 조회·한 장소부터 일정 시작, 민준/openapi 독립 체험·현재 로그인 중 수정 유지·재로그인 초기화, 공통 20초/5단계 가이드·상단 재열기, 개인 계정의 실제 초대 구분. 관광공사 실사용은 국문·무장애 2종 그대로다.
- 표지·순서·폰트·도형 좌표/크기·표·이미지 프레임을 보존했다. 13개 미변경 페이지는 기존 v3.2 PDF와 렌더 픽셀이 동일하다. 변경 5쪽은 전체 크기로 확인했고 18쪽을 다시 렌더했다. PPTX 구조·레이아웃·재열기 통과, PDF **1,823,726바이트**, Pretendard 임베딩, 10MB 이하. [검수](../reports/qa/description-v3-3/review.json).
- 실제 운영에서 독립 openapi 체험으로 로그인/가이드/장소 찾기를 촬영했다. 기존 계정·여행·그룹 편집, 개인 소셜 동의 반복, 앱 변경·새 배포는 없다. 운영 **v23/env5**, source `eb2a6d30b3cb017e0f1c4338d88243cb56d99149`, 개발 **v6/env2** 유지.
- 문서 구현 commit **`5cb090d`**, 작업 branch `jun/description-v3-3`. 문서만 변경하므로 별도 PR 없이 master에 fast-forward하고 이 인계 기록까지 push한다. 최종 SHA/동기화는 `git log -1`, `git status -sb`, `origin/master`를 따른다. 앱 검사를 로컬에서 불필요하게 반복하지 않았으며 문서 생성·검증 스크립트와 실제 캡처를 추적했다.
- v3.2·기존 제출 ZIP은 당시 자료로 보존한다. 사용자가 1차 심사 완료를 알린 상태를 유지하며 포털 파일 교체/재접수를 진행하지 않았다. 다른 PC에서는 새 v3.3 PDF/PPTX를 바로 읽고, 재제작 시 [문서 도구 안내](../scripts/submission/README.md)를 따른다.

**다음 첫 행동:** 후속 설명·발표에는 v3.3을 기준으로 사용한다. 실제 장병·동행자의 첫 일정·그룹 공유 관찰, 외부 네트워크 첫 접속 지연과 운영 DB 전체 복구 준비는 기존 로드맵대로 이어간다.

# 9/21 공개 테스트 계정별 독립 체험 운영 반영
