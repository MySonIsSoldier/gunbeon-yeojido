...
**구매 도메인 전환 준비:** [개발·운영 분리 및 DNS/OAuth 주소 안내](custom-domain-setup.md)를 먼저 확인하세요. 운영 v18의 기존 콜백과 키는 유지 중이며, 새 도메인 철자 확인·DNS/HTTPS·제공자 콜백 등록 후 함께 전환합니다. 아래 기존 주소를 지금 일괄 삭제하거나 AUTH_BASE_URL만 먼저 바꾸지 않습니다. 개발 전용 사이트는 별도 DB이며 Google/Naver 키가 아직 없습니다.
