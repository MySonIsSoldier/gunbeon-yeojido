...
try{
 ...
 await p.getByRole('button',{name:'민준 테스터 계정 선택'}).click();await p.getByRole('button',{name:'민준으로 체험 시작'}).click();await p.getByRole('heading',{name:'일정은 한눈에, 편집은 필요할 때.'}).waitFor();await shot('02-welcome');await p.getByRole('button',{name:'기능별로 따라 해볼게요'}).click();await shot('03-guide');
