...
[Google·Naver 연결 방법](docs/social-login-setup.md) · [네이버 검수 캡처·제출 안내](output/naver-review/README.md) · [계정 구현·검증](reports/accounts_delivery.md)
...
2. **날짜 없이 둘러보기** — 강원 접경 5군과 춘천·속초의 추천 코스 36개를 지역·취향으로 살펴봅니다. 코스를 고른 뒤 일정표에서 날짜·시간을 정합니다. 둘러보기는 현재 시각과 개인 복귀 조건을 사용하지 않습니다.
...
6. **출타와 기록** — 내 여행에 담은 뒤 개인 복귀 기준을 정하고 ‘출타 시작’을 누릅니다. 여행이 끝나면 실제 다녀온 관광지를 선택하고 스탬프를 남깁니다. ‘하루의 한 장’은 이미지·문구로 저장할 수 있습니다. 모든 사용자가 상단의 ‘여행 가이드 다시 보기’에서 기능별 사용법을 먼저 열고, 20초 애니메이션을 선택할 수 있습니다.
...
9/15 운영 TourAPI 철원 **5유형114곳**과 Kakao 지도 동작을 확인했습니다. 원문 응답은 DB에 적재하지 않고 장소 참조만 계획에 보관합니다. TypeScript·99개 단위 검사·브라우저 CI를 통과했습니다. 실제 휴대폰 OS·사용자 관찰·최종 기능설명서·접수는 다음 작업입니다.

## 구현 범위

- 개인 계정·서버 저장·기기 여행 가져오기, Google·네이버 로그인과 계정 연결, 여행 카드 더보기 메뉴
- 공개 관광지 제안·작성자 검토/반영·새 여행 가져오기, 인스타 PNG/링크·X 공유, 기록 수정/계획 복원
- 하루 여권 표지·여유 조정 비교/되돌리기·실제 방문 선택·공개 기록 카드·상단에서 다시 여는 20초·단계별 사용법
...
개인 아이디 로그인과 계정별 서버 저장을 지원합니다. 9/15 네이버 운영 연결과 기존 회원의 실제 재로그인을 확인하고 검수용 캡처를 준비했습니다. 9/15 Google·네이버 실로그인과 기존 여행 복원을 확인했으며 이후 사용자가 두 제공자의 승인·연결 완료를 확인했습니다. 이번 최종 제출 점검에서는 개인 소셜 동의를 반복하지 않았습니다. 비밀번호 재설정 메일은 아직 없습니다. 게스트 참여는 브라우저 쿠키를 유지해야 합니다. 초대는 72시간 동안 유효하고 관리자에게 취소·멤버 제외·그룹 삭제 기능이 있습니다.
