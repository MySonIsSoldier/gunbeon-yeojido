...
const OUT=process.env.SUBMISSION_BUILD_DIR || 'tmp/submission-redesign';
...
sl=flows[0];feature(sl,1,names[0],'1/2 · 장소 검색과 코스 구성','추천 코스를 가져오거나 빈 일정에서\n시작해 관광지·음식점·숙박을\n검색하고 추가합니다.');
...
sl=flows[1];feature(sl,1,names[0],'2/2 · 시간표 확인과 일정 편집','장소의 방문 순서와 머무는 시간을\n정하면 일정표에 반영됩니다.\n일정 보기와 편집 화면을 구분합니다.');
...
sl=flows[6];feature(sl,5,names[4],'1/2 · 공개할 장소와 질문 선택','여행의 관광지와 질문을 링크로\n공유합니다. 방문자는 장소의 추가·\n교체·생략을 구체적으로 제안합니다.');
...
sl=flows[7];feature(sl,5,names[4],'2/2 · 작성자 검토와 실제 일정 반영','작성자가 제안을 검토하고 일정을\n저장하면 반영 완료로 표시됩니다.\n방문자는 공개안으로 새 여행을 만듭니다.');
