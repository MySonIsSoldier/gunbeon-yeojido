"""Export a finalized v3 PPTX with bundled Pretendard, then render every PDF page.
...
root = Path.cwd()
build = Path(os.getenv('SUBMISSION_BUILD_DIR', 'tmp/submission-v3')).resolve()
pptx = Path(os.getenv('SUBMISSION_FINAL_PATH', 'output/submission/2026-round1-v3/gunbeon-2026-round1-functions-v3.2.pptx')).resolve()
pdf = Path(os.getenv('SUBMISSION_PDF_PATH', 'output/pdf/gunbeon-2026-round1-functions-v3.2.pdf')).resolve()
...
writer.add_metadata({'/Title':'군번여지도 강원 · 2026 관광데이터 활용 공모전 기능설명서', '/Author':'우리아들이군인인데여행계획을준비했대', '/Subject':'① 웹·앱 개발 부문 · 1차 심사 기능설명서 v3'})
pdf.parent.mkdir(parents=True,exist_ok=True)
with pdf.open('wb') as f: writer.write(f)
assert pdf.stat().st_size < 10_000_000
review=build/'final-pdf-v3';review.mkdir(exist_ok=True)
subprocess.run(['pdftoppm','-scale-to','1600','-png',str(pdf),str(review/'page')],check=True)
summary={'pdf':str(pdf.relative_to(root)), 'pages':18,'bytes':pdf.stat().st_size,'embeddedFonts':sorted(font_names),'allFontsEmbedded':True,'exportFontconfig':str(config.relative_to(root)), 'renderDirectory':str(review.relative_to(root))}
(build/'pdf-export-v3.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n')
