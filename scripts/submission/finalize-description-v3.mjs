...
const root=process.cwd(), build=process.env.SUBMISSION_BUILD_DIR || 'tmp/submission-v3';
...
const tables=[1,3,11];
const finalPath=process.env.SUBMISSION_FINAL_PATH || path.join(root,'output/submission/2026-round1-v3/gunbeon-2026-round1-functions-v3.2.pptx');
...
const result=await finalizePresentation({
 ...
 receiptPath:process.env.SUBMISSION_RECEIPT_PATH || path.resolve(build,'final-validation-v3.2.json')
});
