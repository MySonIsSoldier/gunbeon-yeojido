"""Package the visually reviewed v3 deck; preserve the historical v1 kit."""
...
root=Path.cwd();build=root/'tmp/submission-v3'
out=root/'output/submission/2026-round1-v3'
pdf=root/'output/pdf/gunbeon-2026-round1-functions-v3.2.pdf'
pptx=out/'gunbeon-2026-round1-functions-v3.2.pptx'
review_path=root/'reports/qa/submission-2026-09-17/description-v3-review.json'
review=json.loads(review_path.read_text())
assert review['pagesReviewed']==list(range(1,19)) and review['status']=='passed'
...
reader=PdfReader(pdf);assert len(reader.pages)==18 and pdf.stat().st_size<10_000_000
...
for src,dest in [('requirements.md','REQUIREMENTS.md'),('portal-copy.md','PORTAL-COPY.md'),('final-checklist.md','CHECKLIST.md'),('rehearsal.md','REHEARSAL.md'),('image-sources.md','IMAGE-SOURCES.md'),('editorial-review-v3.md','EDITORIAL-REVIEW.md')]:
...
(out/'README.md').write_text(f'''# 군번여지도 강원 · 1차 기능설명서 v3.2
...
| 파일 | 용도 |
|---|---|
| [gunbeon-2026-round1-functions-v3.2.pdf](gunbeon-2026-round1-functions-v3.2.pdf) | 제출용 기능설명서. 18쪽, {pdf.stat().st_size/1e6:.2f}MB, 10MB 이하 |
...
with ZipFile(pptx) as z:
 ...
 assert len(slides)==18 and all(tables[str(i)]>=1 for i in [1,3,11])
receipt=json.loads((build/'final-validation-v3.2.json').read_text());export=json.loads((build/'pdf-export-v3.json').read_text())
validation={'preparedAt':datetime.now(timezone.utc).isoformat(),'revision':'v3.2','competitionTrack':'① 웹·앱 개발 부문','deadline':'2026-09-21T16:00:00+09:00','applicationSubmitted':False,'production':{'url':'https://gunbeon.gangwon.kr','version':'v20/env5','sourceSha':'567ea91c800b3077be41c7bf68fc88b5e2bb7409','deploymentConfirmedAt':'2026-09-17T05:12:22Z'},'pdf':{**info(pdf),'pages':18,'under10MB':True,'exportedFromFinalPptxSha':sha(pptx),'allTextFontsEmbedded':export['allFontsEmbedded'],'fontFamilies':['Pretendard Regular','Pretendard SemiBold']},'pptx':{**info(pptx),'slides':18,'nativeTableCounts':tables,'packageIntegrity':receipt['packageIntegrity']['status'],'artifactToolReimportPassed':receipt['firstPartyImport']['passed'],'fontPolicyPassed':receipt['fontSelection']['passed']},'template':{'sha256':sha(source),'originalSlides':9,'flowSlidesExpandedTo':10,'originalMediaCount':len(source_media),'byteIdenticalRetainedMediaCount':len(source_media & final_media),'requiredSectionsPreserved':True},'visualReview':review,'images':{'representative':1,'details':3,'logo140':1,'manifest':'selected-images.json','creditFile':'IMAGE-SOURCES.md'},'evidence':{'api':'reports/qa/submission-final-2026-09-17/live-api.json','screens':'reports/qa/submission-2026-09-16/screens/meta.json','newScreens':'reports/qa/tester-social/scenarios/result.json','noApiMocks':True,'notPhysicalDeviceTesting':True,'submissionReview':'reports/final_submission_review_2026-09-17.md'},'remaining':['Final team members and applicant email confirmation','Actual phone final rehearsal','Private API key/account fields and final portal submission','Submission receipt retention','Hosting/database recovery owner confirmation']}
dump(out/'validation.json',validation)
archive=root/'output/submission/gunbeon-2026-round1-kit-v3.zip'
with ZipFile(archive,'w',ZIP_DEFLATED,compresslevel=9) as z:
 for file in sorted(out.rglob('*')):
  if file.is_file():z.write(file,'gunbeon-2026-round1-v3/'+str(file.relative_to(out)))
